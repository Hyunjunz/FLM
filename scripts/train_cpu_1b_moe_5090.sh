...
# RTX 5090 32GB preset for the CPU-target 1B+ top-1 MoE model.
# Start conservative; if VRAM is below ~29GB, raise BATCH_SIZE to 6 or 8.
# Override example:
#   BATCH_SIZE=6 GRAD_ACCUM_STEPS=12 NUM_WORKERS=6 bash scripts/train_cpu_1b_moe_5090.sh
...
OUTPUT_DIR="${OUTPUT_DIR:-artifacts/cpu_1b_moe_5090_ckpt}"
...
echo "Starting 1B+ MoE training for RTX 5090"
echo "  data=$DATA"
echo "  config=$CONFIG"
...
echo "  workers=$NUM_WORKERS prefetch_factor=$PREFETCH_FACTOR"
